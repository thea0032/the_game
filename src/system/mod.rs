use crate::{location::Location, systems::{Systems, object_id::ObjectID}, ui::ansi};
...
impl System{
    pub fn new(name:String, location:Location) -> System{
        System{
            name:name, loc:location, objs:vec![],
        }
    }//Basic constructor
    ...
    pub fn display(&self, amt_before:usize, names:&Vec<String>, sys:&Systems) -> String{
        ...
        for i in 0..self.objs.len(){
            ...
            res.push_str(&format!("{}. {}\n", i + amt_before, names[self.objs[i].get()]));
        }
        return res;
    ...
    pub fn display_filtered(&self, amt_before:usize, will_display:&Vec<bool>, names:&Vec<String>) -> String{
        ...
        for d in will_display{
            if *d{//If we want to display this...
    ...
    pub fn get_objs<'a>(&'a self) -> &'a Vec<ObjectID>{
        return &self.objs;
    }//Getter
    pub fn color(&self, sys:&Systems) -> &str{
        ...
        if is_green{
            return ansi::GREEN;//If it isn't all yellow and there's a green object, returns green
        }
        return ansi::BLUE;//Otherwise, returns blue
