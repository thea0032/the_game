...
use super::{ansi, instr, io::{get_from_input_valid, wait_for_input}};
...
pub fn quickie(rss:&ResourceDict, cmp:&Components, sys:&mut Systems, dir:&mut Quickie, obj:ObjectID){
    loop{
        ...
        let input:usize = get_from_input_valid("", "Please enter a valid input", |x| *x < len + dir.len());//Gets option
        match input{
            0=>break,//Breaks out of menu
            1=>{
                println!("{}", dir.display(0, obj, sys, rss, cmp));
                println!("{}. Add on end", dir.len());//Displays options
                let pos = get_from_input_valid("Enter the place where you want to add an instruction", "Please enter a valid number!", |x| *x <= dir.len());//Gets position
                let instr = instr::make_instr(rss, cmp, sys, obj, dir.len() + 1);//Gets instruction
                if let Some(val) = instr{//If the instruction creation wasn't aborted
            ...
            2=>{
                ...
                wait_for_input("Press enter to continue: ");//Same as before, but the instruction is temporary
            },
            3=>{
                println!("{}", dir.display(0, obj, sys, rss, cmp));
                println!("{}{}. abort", ansi::RED, dir.len());//Displays options
                let pos = get_from_input_valid("Enter the place where you want to remove an instruction: ", "Please enter a valid number!", |x| *x <= dir.len());//Gets input
                if pos < dir.len(){//If it wasn't aborted...
                    dir.rmv(pos);//removes
            ...
            _=>{
                ...
                instr::instr_menu(rss, cmp, sys, obj, &mut dir.get(input - 4), len);//Enters instruction menu
            }
        }//Responds to options
    }
}