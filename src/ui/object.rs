...
pub fn object_menu(
    ...
    loop {
        ...
        match response {
            ...
            _ => {
                io::get_str("Something went horribly wrong!");
...
pub fn get_object(sys: &Systems, curr_sys: SystemID) -> ObjectID {
    println!(
        "{}",
        sys.get_s_stat(curr_sys).display(0, sys.get_o_names(), sys)
    ); //Displays all objects in system
    let input: usize =
        get_from_input_valid("Enter the object: ", "Please enter a valid number", |x| {
