...
impl Object{
    ...
    pub fn pop_benefits(&mut self){
        ...
        self.resources.add_res(constants::PRODUCTION, pops);//Produces 1 production
    ...
    pub fn pop_upkeep(&mut self) -> bool{
        ...
        for (rss, fct) in Object::UPKEEP_RSS_POPS{//For each bit of upkeep...
            let upkeep = rand::rand_round(self.resources.get_curr(constants::POPULATION) as f64 * fct);//Gets the amount of upkeep required
            if !self.resources.rmv_res(*rss, upkeep){//If we don't have enough of the resource, and can't afford all of our upkeep...
    ...
    pub fn plant_upkeep(&mut self) -> bool{
        let mut satisfied:bool = true;
        for (rss, fct) in Object::UPKEEP_RSS_PLANTS{//For each bit of upkeep...
