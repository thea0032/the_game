...
impl FileObject {
    pub fn read_from(file: Vec<String>, name: String) -> FileObject {
        ...
        for mut line in file {
            if line.len() >= WHITESPACE_LEN && &line[0..WHITESPACE_LEN] == WHITESPACE {
                ...
                if temp.len() == 1 {
                    contents.push(FileObject::read_from(buffer, buffer_name));
        ...
        if contents.len() != 0 {
            contents.push(FileObject::read_from(buffer, buffer_name)); //Adds the last bit
